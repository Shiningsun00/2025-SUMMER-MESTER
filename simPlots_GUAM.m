...
%% Cost Function 
err = pos - pos_des;    % value - predicted value
e2 = sum(err.^2, 2);    % 2-dimension

%%  Drawing plot
fRMS_t = sqrt(mean(err.^2, 2));  % Nx3 err -> 시각별 RMS

figure; 
hold on; grid on; box off;

plot(time, fRMS_t, 'b-', 'LineWidth', 2, 'DisplayName', 'RMS error');

local_max_time = [];
local_max_val  = [];

for i = 2:length(time)-1
    if fRMS_t(i) > fRMS_t(i-1) && fRMS_t(i) > fRMS_t(i+1)
        local_max_time(end+1) = time(i);
        local_max_val(end+1)  = fRMS_t(i);
        h = plot([time(i) time(i)], [0 fRMS_t(i)], 'g-', 'LineWidth', 1);
        h.Annotation.LegendInformation.IconDisplayStyle = 'off';
        fprintf('Local maximum error occurs at t =  %.4f\n', time(i));
    end
end

plot(local_max_time, local_max_val, 'ro', 'MarkerSize', 8, ...
    'MarkerFaceColor', 'y', 'DisplayName', 'Local MAX');

% 축 라벨, 제목, 폰트
xlabel('Time [s]', 'FontSize', 14, 'FontWeight', 'bold');
ylabel('RMS position error', 'FontSize', 14, 'FontWeight', 'bold');
title('RMS error vs Time', 'FontSize', 16, 'FontWeight', 'bold');
...
% --- 고도 추종 오차 플롯 ---
altitude_err = pos(:,3) - pos_des(:,3);

figure; hold on; grid on; box off;

% 메인 고도 오차 곡선
plot(time, altitude_err, 'r-', 'LineWidth', 2, 'DisplayName', '\Delta altitude');
...
for i = 2:length(time)-1
    if altitude_err(i) > altitude_err(i-1) && altitude_err(i) > altitude_err(i+1)
        local_max_time(end+1) = time(i);
        local_max_val(end+1)  = altitude_err(i);
        % 짧은 수직선 표시
        h = plot([time(i) time(i)], [0 altitude_err(i)], 'g-', 'LineWidth', 1);
        h.Annotation.LegendInformation.IconDisplayStyle = 'off';
        fprintf('Local maximum error occurs at t =  %.4f\n', time(i));
    end
end
...
for i = 2:length(time)-1
    if altitude_err(i) < altitude_err(i-1) && altitude_err(i) < altitude_err(i+1)
        local_min_time(end+1) = time(i);
        local_min_val(end+1)  = altitude_err(i);
        % 짧은 수직선 표시
        h = plot([time(i) time(i)], [0 altitude_err(i)], 'g-', 'LineWidth', 1);
        h.Annotation.LegendInformation.IconDisplayStyle = 'off';
        fprintf('Local minimum error occurs at t =  %.4f\n', time(i));
    end
end

% 국소 최대 지점 마커 표시
plot(local_min_time, local_min_val, 'ro', 'MarkerSize', 8, ...
    'MarkerFaceColor', 'y', 'DisplayName', 'Local MAX');

% 축 설정
xlabel('Time [s]', 'FontSize', 14, 'FontWeight', 'bold');
ylabel('\Delta altitude', 'FontSize', 14, 'FontWeight', 'bold');
title('Altitude tracking error vs Time', 'FontSize', 16, 'FontWeight', 'bold');
