...
while sim_time < SIM.end_time
    ...
    if sim_time < SIM.end_time/6
        state.pn = state.pn + 5*SIM.ts_simulation;
    elseif sim_time < 2*SIM.end_time/6
        state.theta = state.theta + 0.1*SIM.ts_simulation;
        state.pn = state.pn + 5*SIM.ts_simulation;
        state.h = state.h + 4.8*SIM.ts_simulation;
    elseif sim_time < 4*SIM.end_time/6
        state.theta = state.theta - 0.05*SIM.ts_simulation;
        state.pn = state.pn + 10*SIM.ts_simulation;
    else
