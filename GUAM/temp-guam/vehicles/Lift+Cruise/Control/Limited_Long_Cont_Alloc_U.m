function u = Limited_Long_Cont_Alloc_U(u_lon, eng_max, eng_min, act_max, act_min)
...
if any(u_act(1:9) > eng_max)
    for loop = 1:9
        scale_temp = eng_max(loop)/(u_act(loop));
...
if u_act(10)>act_max(3)
    scale_temp = act_max(3)/(u_act(10));
    if scale_temp>0 && scale_temp<1 && scale_temp<scale_factor
        scale_factor = scale_temp;
    end
end
if u_act(10)<act_min(3)
    scale_temp = act_min(3)/(u_act(10));
    if scale_temp>0 && scale_temp<1 && scale_temp<scale_factor
        scale_factor = scale_temp;
    end
end
...
if u_act(11)>act_max(1)
    scale_temp = act_max(1)/(u_act(11));
    if scale_temp>0 && scale_temp<1 && scale_temp<scale_factor
        scale_factor = scale_temp;
    end
end
if u_act(11)<act_min(1)
    scale_temp = act_min(1)/(u_act(11));
    if scale_temp>0 && scale_temp<1 && scale_temp<scale_factor
        scale_factor = scale_temp;
    end
end
